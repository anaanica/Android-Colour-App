package hr.algebra.colourapp.framework
...
fun Context.fetchItems() : MutableList<Item> {
    ...
    val cursor = contentResolver.query(
        COLOUR_PROVIDER_CONTENT_URI, null, null, null, null
    )
    while (cursor != null && cursor.moveToNext()){
        items.add(Item(
            ...
            cursor.getString(cursor.getColumnIndexOrThrow(Item::username.name)),
            cursor.getString(cursor.getColumnIndexOrThrow(Item::picturePath.name)),
            cursor.getString(cursor.getColumnIndexOrThrow(Item::hexadecimal.name)),
